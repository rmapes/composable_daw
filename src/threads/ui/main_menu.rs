...
pub fn top_menu_view() -> Element<'static, Message> {
    // Define menu levels
    let menu_tpl_1 = |items: Vec<Item<'static, Message, iced::Theme, iced::Renderer>>| {
        Menu::new(items).width(180.0).offset(15.0).spacing(5.0)
    };
    ...
    menu_bar!(
        ...
        (
            ...
            menu_tpl_1(menu_items!(
                // (text("New"))
                // (text("Open"))
                (button(text("Add Pattern"))
                    .on_press(Message::AddRegionAtPlayhead(RegionType::Pattern))),
