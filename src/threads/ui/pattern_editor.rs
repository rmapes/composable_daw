use iced::widget::{Column, Container, Row, button, row, text};
use iced::{Element, Length};

const NOTE_BUTTON_SIZE: f32 = 20.0;
use super::super::engine::actions::Actions;
use crate::models::sequences::PatternSeq;
use crate::models::shared::{PatternNoteIdentifier, RegionIdentifier};
...
impl Component {
    ...
    pub fn view(&self, pattern: &PatternSeq) -> Element<'_, Message> {
        ...
        components::module(content.width(self.width).height(self.height).into())
            .id("PatternEditor")
            .into()
    }
}
...
fn pattern_editor_row(pattern: &PatternSeq, note_num: u8) -> Row<'static, Message> {
    ...
    let beat_toggles: Vec<Element<'static, Message>> = (0..pattern.num_beats)
        .map(|beat_num| {
            note_toggle_button(
                ...
                note_num,
                beat_num,
                pattern.is_on(beat_num, note_num),
            )
            .into()
        })
        .collect();
    ...
    let label = note_label(
        pattern
            ...
            .expect("Unexpected index into note_values"),
    );
...
fn note_label(midi_pitch: &u8) -> Element<'static, Message> {
    components::label(
        text(format!("{midi_pitch}"))
            .width(Length::Fixed(NOTE_BUTTON_SIZE))
            .height(Length::Fixed(NOTE_BUTTON_SIZE))
            .into(),
    )
    .into()
}
...
fn note_toggle_button(
    pattern_id: RegionIdentifier,
    note_num: u8,
    beat_num: u8,
    is_on: &bool,
) -> Container<'static, Message> {
    let style = if *is_on {
        style::note_button_on
    } else {
        style::note_button_off
    };
    components::control(
        button(
            Container::new(row![])
        ...
        .on_press(Message::Engine(Actions::PatternClickNote(
            PatternNoteIdentifier {
