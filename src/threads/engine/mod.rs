    ...
    thread::spawn({
        ...
        move || {
            let (audio, stereo_output) = match audio::init_audio(&tx.clone()) {
                ...
                Err(e) => {
                    error!(
                        "Failed to initialize audio: {}. Continuing without audio output.",
                        e
                    );
                    let dummy_audio = audio::AudioEngine::dummy(44100);
                    let (_consumer, dummy_stereo_output) =
                        audio::controllers::stereo_output::StereoOutputController::new();
                    if let Ok(mut state) = player_state.write() {
            ...
            let mut audio_sources = AudioSources::new(
                audio,
                stereo_output,
                &project.tracks,
            ...
            loop {
                ...
                let follow_up = match received {
                    actions::Actions::Play => {
                        ...
                        if let Ok(mut state) = player_state.write() {
                            if ALWAYS_PLAY_FROM_START && !state.is_playing {
                                ...
                                state.samples_played = (state.playhead * state.sample_rate
                                    / project.ticks_per_second())
                                    as usize;
                        ...
                        match project.tracks[src_idx].remove_region(&region_id) {
                            Some(sequence) => {
                                ...
                                match target_track.insert_region(new_tick, sequence) {
                                    Ok(()) => {
                                        if let Err(e) =
                                            audio_sources.update_track(&project.tracks[src_idx])
                                        {
                                            error!(
                                                "FATAL: Unexpected error updating source track: {}",
                                                e
                                            );
                                            ActionFollowUp::Exit
                                        } else if src_idx != tgt_idx
                                            && audio_sources
                                                .update_track(&project.tracks[tgt_idx])
                                                .is_err()
                                        {
                                    ...
                                    Err((_, sequence)) => {
                                        if project.tracks[src_idx]
                                            .insert_region(region_id.region_id, sequence)
                                            .is_err()
                                        {
                                            error!(
                                                "FATAL: Failed to restore region after move collision"
                                            );
                                            ActionFollowUp::Exit
                                        } else if let Err(e) =
                                            audio_sources.update_track(&project.tracks[src_idx])
                                        {
                                            error!(
                                                "FATAL: Unexpected error restoring track: {}",
                                                e
                                            );
                                            ActionFollowUp::Exit
                                        } else {
                    ...
                    actions::Actions::DeleteMultipleMidiNotes(
                        region_identifier,
                        notes_to_delete,
                    ) => {
                        let track = &mut project.tracks[region_identifier.track_id.track_id];
                    ...
                    actions::Actions::UpdateMidiNote(
                        region_identifier,
                        old_start_tick,
                        note_index,
                        new_start_tick,
                        ...
                        match sys_ev {
                            actions::SystemActions::SamplesPlayed(num_samples) => {
                                    ...
                                    && state.is_playing
                                {
                                    ...
                                    let new_playhead = project.ticks_per_second()
                                        * state.samples_played as u32
                                        / state.sample_rate;
                                    if new_playhead != state.playhead {
                                        ...
                                        if audio_sources.should_stop_playback(new_playhead) {
                                            info!(
                                                "Playback finished - last note off processed, stopping playback"
                                            );
                                            state.is_playing = false;
                                        }
                                    }
